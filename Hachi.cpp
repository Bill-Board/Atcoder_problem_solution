...
/// find_by_order(k) – kth index a ki lagbe, pointer return korbe.
/// order_of_key(x) – x kon position a lagbe , oita bole dibe
...
///-------------------------------------------------------------------------------------------------------
vll cnt(10);
bool alabu(vll lagbe)
{
    for(v99 i=0;i<cnt.size();i++)if(cnt[i]<lagbe[i])return false;
    return true;
}
void solve()
{
    ...
    if(s.size()<3)
    {
        if(stol(s)%8==0)return print("Yes");
        reverse(sob(s));
        if(stol(s)%8==0)return print("Yes");
        print("No");
        return;
    ...
    for(v99 i=112;i<=1000;i+=8)
    {
        vll lagbe(10);
        for(auto j:to_string(i))lagbe[j-'0']++;
        if(alabu(lagbe))return print("Yes");
    }
    print("No");
}
