...
v99 a[1002],b[1002], dist[1002][1002];
vector<pii>vp[1003];
...
void solve()
{
    ...
    v99 n,m;
    read(n,m);
    rep1(i,1,n)rep1(j,1,n)dist[i][j]=INT_MAX;
    rep1(i,1,m)
    {
        char c;read(a[i],b[i],c);
        vp[a[i]].pb({b[i],c});
        vp[b[i]].pb({a[i],c});
    }
    queue<pii>q;
    ...
    while(q.size())
    {
        v99 a1=q.front().ff,b1=q.front().ss;q.pop();
        for(auto i:vp[a1])for(auto j:vp[b1])
        {
            if(i.ss!=j.ss)continue;
            if(dist[i.ff][j.ff]==INT_MAX)
            dist[i.ff][j.ff]=dist[a1][b1]+1,q.push({i.ff,j.ff});
        }
    }
    v99 cr_h=LLONG_MAX;
    rep1(i,1,n)cr_h=min(cr_h,2*dist[i][i]);
    rep1(i,1,m)cr_h=min({cr_h,2*dist[a[i]][b[i]]+1,2*dist[b[i]][a[i]]+1});
    print(cr_h);
}
