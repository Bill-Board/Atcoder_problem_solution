...
void solve()
{
    ...
    string s;read(s);
    vll a;v99 p=0;
    while(s[p]=='0' and p<n)p++;
    if(p==n)return print(s);
    for(v99 i=p-1,k=m;k and ~i;k--,i--)s[i]='1';
    v99 pp=n-1;
    while(s[pp]=='0' and pp>=0)pp--;
    for(v99 i=pp+1,k=m;k and i<n;k--,i++)s[i]='1';
    for(v99 i=p;i<pp;)
    {
        while(s[i]=='1' and i<pp)i++;
        v99 st=i;
        while(s[i]=='0' and i<pp)i++;
        v99 en=i-1,kot=m;
        while(kot--)
        {
            if(en==st)break;
