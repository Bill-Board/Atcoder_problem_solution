...
void precal()
{
    ...
    for(v99 i=1;i<=N;i++) fact[i]=(fact[i-1]*i)%mod;
    inv[N]=bigmod(fact[N],mod-2,mod);
    for(v99 i=N-1;~i;--i)inv[i]=(inv[i+1]*(i+1))%mod;
}
v99 ncr(v99 n,v99 r)
{
    v99 age=( (inv[n-r]%mod) * (inv[r]%mod) )%mod;
    v99 kot=(fact[n]*age)%mod;
    return kot;
}
...
void solve()
{
    ...
    v99 cr_h=0;
    for(v99 i=0;i<=n;i+=2)
        cr_h=(cr_h+ncr(n,i))%mod;
    if(n&1)
    {
        cr_h=(cr_h+1)%mod;
        print(bigmod(cr_h,k,mod));
        return;
    }

    cr_h=bigmod(cr_h,k,mod);
    v99 ans=0;print(cr_h);
    for(v99 i=1;i<k;i++)
    {
        ans=(ans+ bigmod(2LL,n*(k-i-1),mod) +mod)%mod,print(n*(k-i-1));
    }
    print(ans+cr_h);
    ...
    for(v99 i=0;i<8;i++)
    {
        for(v99 j=0;j<8;j++)
        {
            ...
            {
                for(v99 l=0;l<8;l++)
                    if((i&j&k&l)>=(i^j^k^l))p++;
            }
            //if((i&j)>=(i^j))p++,print(i,j);
        }
    }
    print(p);
}
