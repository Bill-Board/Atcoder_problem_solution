...
///-------------------------------------------------------------------------------------------------------
void s_jog(string &s,string p)
{
    v99 carry=0;
    for(v99 i=s.size()-1,j=p.size()-1;i>=0;i--,j--)
    {
        v99 ek=s[i]-'0' +carry,dui=0;
        if(j>=0)dui=p[j]-'0';
        if(ek+dui<10)carry=0;
        else carry=1;
        s[i]=char((ek+dui)%10 +'0');
    }
}
...
void solve()
{
    ...
    q=s;
    s="00"+s;
    for(v99 i=0,j=0;i<n-1 and j<100;i++,j++)
    {
        p+=q[i];
        s_jog(s,p);
    }
