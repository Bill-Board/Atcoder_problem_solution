...
///-------------------------------------------------------------------------------------------------------
bool is_stright_line(v99 x1,v99 y1,v99 x2,v99 y2,v99 x3,v99 y3)
{
    v99 a = x1 * (y2 - y3) +x2 * (y3 - y1) +x3 * (y1 - y2);
    return a==0;
}
...
void solve()
{
    ...
    v99 n;read(n);
    vector<pii>vp(n);
    rep0(i,0,n)read(vp[i].ff,vp[i].ss);
    for(v99 i=0;i<n;i++)
    {
        for(v99 j=i+1;j<n;j++)
        {
            for(v99 k=j+1;k<n;k++)
            {
                if(is_stright_line(vp[i].ff,vp[i].ss,vp[j].ff,vp[j].ss,vp[k].ff,vp[k].ss))
                    return print("Yes");
            }
        }
    }print("No");
