...
{
    /// code is here->
    v99 n,m,mod=1e9+7;read(n,m);
    vll path(n+2,0),dist(n+2,-1),tom[n+2];
    rep0(i,0,m)
    {
        v99 x,y;read(x,y);
        tom[x].pb(y);
    ...
    while(q.size())
    {
        v99 city=q.front();q.pop();
        for(auto nig:tom[city])
        {
            if(dist[nig]==-1)/// not visited
            {
                q.push(nig);
                dist[nig]=dist[city]+1;
                path[nig]=path[city];
            }
            else if(dist[city]+1==dist[nig])
                path[nig]+=path[city],path[nig]%=mod;
        }
    }
    print(path[n]%mod);
}
