...
{
    if(n<10) return 10LL;
    if(n<100)return 100LL;
    if(n<1000) return 1000LL;
    if(n<10000) return 10000LL;
    if(n<100000) return 100000LL;
    if(n<1000000) return 1000000LL;
    else return 10000000LL;
}
bool ok(v99 n)
{
    if(n>9 and n<100)return true;
    if(n>999 and n<10000)return true;
    if(n>99999 and n<1000000)return true;
    return false;
}
bool ok2(v99 n)
{
    string p=to_string(n);
    for(v99 i=1;i<p.size();i++) if(p[i]!=p[i-1])return false;
    return true;
}
...
void solve()
{
    /// code is here->
    v99 n;read(n);
    for(v99 i=1LL;i<=1000010LL;i++)
    {
        if(ok(i) and ok2(i) and i<=n) s.insert(i);
        if(i*alabu(i) +i <=n)s.insert(i*alabu(i) +i);
    }
    print(s.size());
}
