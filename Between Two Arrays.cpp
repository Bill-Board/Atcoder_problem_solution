...
{
    /// code is here->
    v99 n,mod=998244353;read(n);
    vll a(n),b(n),ans(3010);read(a);read(b);
    for(v99 i=b[n-1];i>=a[n-1];i--)ans[i]+=(ans[i+1]+1);
    for(v99 i=n-2;i>=0;i--)
    {
        for(v99 j=a[i];j<a[i+1];j++)
        {
            v99 ind=a[i+1];
            ans[j]=(ans[ind]);
        }
        for(v99 j=b[i]-1;j>=a[i];j--)ans[j]=(ans[j]+ ans[j+1])%mod;
    }print(ans[a[0]]%mod);
}
