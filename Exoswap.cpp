...
{
    /// code is here->
    v99 n;read(n);v99 a[n+2],v[n+2],ok[n+1];
    rep1(i,1,n)read(a[i]),v[a[i]]=i,ok[i]=1;
    vector<v99>ans;
    for(v99 i=1;i<n;i++)
    {
        v99 pos=v[i];
        while(pos!=i)
        {
            ...
            {
                swap(a[pos-1],a[pos]);
                swap(v[a[pos-1]],v[a[pos]]);
                if(ok[pos-1])ans.pb(pos-1),ok[pos-1]=0;
                else {cout<<-1<<ses;return;}
                pos--;
            }
            else
            {
    ...
    if(ans.size()==n-1)for(auto i:ans)cout<<i<<ses;
    else cout<<-1<<ses;
