...
void solve()
{
    ...
    map<v99,v99>mpp;
    vll a(n);
    rep0(i,0,n)read(a[i]),visi[i]=i;
    for(v99 i=0,j=n-1;i<j;i++,j--)
    {
        if(a[i]!=a[j])tom[a[i]].pb(i),tom[a[j]].pb(j);
    }
    for(v99 i=0,j=n-1;i<j;i++,j--)
    {
        if(a[i]==a[j])continue;
        if(tom[a[i]].size()>=tom[a[j]].size())
        {
            for(auto k:tom[a[j]])a[k]=a[i],tom[a[i]].pb(k);
        }
        else
        {
            for(auto k:tom[a[i]])a[k]=a[j],tom[a[j]].pb(k);
        }
        cr_h++;
    }
    print(cr_h);
