...
///-------------------------------------------------------------------------------------------------------
bool sort_ok1(const pair<pii,v99> &a,const pair<pii,v99> &b)
{
    if(a.ff.ff==b.ff.ff)return a.ff.ss<b.ff.ss;
    return a.ff.ff<b.ff.ff;
}
bool sort_ok2(const pair<pii,v99> &a,const pair<pii,v99> &b)
{
    if(a.ff.ss==b.ff.ss)return a.ff.ff<b.ff.ff;
    return a.ff.ss<b.ff.ss;
}
void solve()
{
    /// code is here->
    v99 h,w,n;read(h,w,n);
    vector<pair<pii,v99> >a(n);
    vector<pii>ans(n);
    rep0(i,0,n)read(a[i].ff.ff,a[i].ff.ss),a[i].ss=i;
    sort(sob(a),sort_ok1);
    for(v99 i=0,j=1;i<n;i++)
    {
        if(i==0)ans[a[i].ss].ff=j;
        else if(a[i].ff.ff==a[i-1].ff.ff)ans[a[i].ss].ff=j;
        else ans[a[i].ss].ff=++j;
    }
    sort(sob(a),sort_ok2);
    for(v99 i=0,j=1;i<n;i++)
    {
        if(i==0)ans[a[i].ss].ss=j;
        else if(a[i].ff.ss==a[i-1].ff.ss)ans[a[i].ss].ss=j;
        else ans[a[i].ss].ss=++j;
    }
    for(auto i:ans)print(i.ff,i.ss);
}
