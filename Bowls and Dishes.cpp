...
void solve()
{
    ...
    v99 n,m,k;read(n,m);
    vector<pii>ab(m);
    for(auto &i:ab)read(i.ff,i.ss);
    read(k);
    vector<pii>cd(k);
    for(auto &i:cd)read(i.ff,i.ss);
    v99 kot=(1<<k),cr_h=0;
    for(v99 i=0;i<kot;i++)
    {
        set<v99>s;
        for(v99 j=0;j<k;j++)
        {
            if(i&(1<<j))s.insert(cd[j].ff);
            else s.insert(cd[j].ss);
        }
        v99 cnt=0;
        for(auto j:ab)if(s.find(j.ff)!=s.end() and s.find(j.ss)!=s.end())cnt++;
        cr_h=max(cr_h,cnt);
    }
    print(cr_h);
}
